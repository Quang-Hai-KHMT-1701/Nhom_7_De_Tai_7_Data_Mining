...
>
> Em xin tóm tắt lại: Project này đã thực hiện phân cụm khách hàng dựa trên luật kết hợp, đạt Silhouette Score 0.99, và phân loại được các nhóm khách hàng có ý nghĩa cho marketing.
>
...
## 1. "Tại sao K=10 mà không phải K khác?"
> **Trả lời:** "Dạ, em đã thử K từ 2 đến 10 và đo Silhouette Score. K=10 cho kết quả cao nhất là 0.9933. Em cũng thử Elbow method nhưng điểm elbow không rõ ràng, nên em dựa vào Silhouette để chọn ạ."

## 2. "Tại sao Cluster 0 chiếm đến 96.6%?"
> **Trả lời:** "Dạ, đây là hiện tượng Long-tail distribution, rất phổ biến trong bán lẻ. Đa số khách hàng có hành vi mua sắm giống nhau nên nằm cùng cluster. Chỉ số ít khách có hành vi đặc biệt thì tách riêng. Điều này hoàn toàn phù hợp với quy luật Pareto: 20% khách tạo 80% doanh thu ạ."

## 3. "Silhouette 0.99 có quá cao không? Có overfitting không?"
> **Trả lời:** "Dạ, Silhouette cao là do em dùng Lift làm trọng số. Các luật có Lift cao (ví dụ 85) tạo ra khoảng cách lớn giữa các cluster, nên Silhouette cao. Đây là kết quả hợp lý vì các luật đã được lọc kỹ, không phải overfitting ạ."

## 4. "Apriori và FP-Growth khác nhau thế nào?"
> **Trả lời:** "Dạ, cả 2 đều tìm frequent itemsets và association rules. Khác nhau ở cách triển khai: Apriori duyệt database nhiều lần nên chậm, FP-Growth xây FP-Tree nên nhanh hơn. Kết quả cuối cùng giống nhau ạ."

## 5. "Support, Confidence, Lift nghĩa là gì?"
> **Trả lời:** "Dạ:
> - Support = % đơn hàng có luật này (ví dụ 1% = 1% đơn hàng)
> - Confidence = Nếu mua A thì khả năng mua B (ví dụ 50% = mua A thì 50% mua B)
> - Lift = So sánh với ngẫu nhiên (ví dụ Lift=85 = cao gấp 85 lần ngẫu nhiên)
> 
> Lift quan trọng nhất vì cho biết luật có ý nghĩa thực sự hay không ạ."

## 6. "PCA là gì? Tại sao dùng PCA?"
> **Trả lời:** "Dạ, PCA là Principal Component Analysis - kỹ thuật giảm chiều dữ liệu. Vì dữ liệu có 50 chiều (50 luật) không vẽ được, em dùng PCA giảm về 2 chiều để visualize. 2 trục PC1 và PC2 giữ được 95% thông tin gốc nên biểu đồ vẫn phản ánh đúng cấu trúc ạ."

## 7. "Tại sao chọn K-Means mà không phải DBSCAN?"
> **Trả lời:** "Dạ, DBSCAN cho Silhouette cao hơn một chút (0.9973), nhưng nó đánh dấu 99 khách là outliers - không thuộc cluster nào. Trong kinh doanh, mỗi khách hàng đều có giá trị, em không muốn bỏ sót ai nên chọn K-Means ạ."

## 8. "Binary và Lift weighting khác nhau thế nào?"
> **Trả lời:** "Dạ:
> - Binary: Nếu khách mua theo luật → ghi 1, không → ghi 0
> - Lift weighting: Nếu khách mua theo luật → ghi giá trị Lift của luật đó (ví dụ 85), không → ghi 0
> 
> Lift weighting tốt hơn vì nó phân biệt được luật quan trọng và luật ít quan trọng ạ."

## 9. "Dữ liệu này có bao nhiêu khách hàng?"
> **Trả lời:** "Dạ, dữ liệu gốc có khoảng 400,000 giao dịch. Sau khi lọc chỉ giữ UK customers và loại bỏ đơn hàng lỗi, em còn lại 3,921 khách hàng ạ."

## 10. "Top-50 luật tốt hơn Top-200 như thế nào?"
> **Trả lời:** "Dạ, em đã thử cả 3 trường hợp:
> - Top-50: Silhouette = 0.99
> - Top-100: Silhouette = 0.92
> - Top-200: Silhouette = 0.85
> 
> Nhiều luật hơn không tốt hơn vì sẽ có nhiều luật kém chất lượng (nhiễu) làm giảm khả năng phân biệt giữa các cluster ạ."
...
| 2 | Bài toán | 30 giây |
| 3 | Dữ liệu | 1 phút |
| 4 | Association Rules | 1 phút 30 giây |
| 5 | Cách chọn luật | 1 phút |
| 6 | 10 luật tiêu biểu | 1 phút |
| 7 | Tạo đặc trưng | 1 phút 15 giây |
| 8 | So sánh đặc trưng | 1 phút |
| 9 | K-Means | 1 phút |
| 10 | Chọn K | 45 giây |
| 11 | PCA | 1 phút |
| 12 | Tổng quan kết quả | 45 giây |
| 13 | Cluster 0 | 1 phút |
| 14 | Cluster 1 | 1 phút 15 giây |
| 15 | Clusters khác | 45 giây |
| 16 | So sánh thuật toán | 1 phút |
| 17 | Kết luận | 1 phút |
| 18 | Q&A | 15 giây |
| **TỔNG** | | **~16 phút** |
...
1. **Đọc file này ít nhất 3 lần** để thuộc script
2. **Chạy thử** trước gương hoặc quay video bản thân
3. **In ra 1 bản** script để cầm trên tay (đề phòng quên)
4. **Kiểm tra laptop** - PowerPoint, hình ảnh đầy đủ

## KHI ĐANG THUYẾT TRÌNH:
1. **Nói CHẬM** - vì hồi hộp hay nói nhanh
2. **Nhìn cô giáo và các bạn** - đừng nhìn màn hình suốt
3. **Nếu quên** → Nhìn slide, slide có keyword để nhắc nhớ
4. **Nếu cô hỏi mà không biết** → "Dạ câu này em chưa nghĩ đến, em xin ghi nhận ạ"

## SAU KHI THUYẾT TRÌNH:
1. **Đừng vội ngồi xuống** - đợi cô cho phép
2. **Nói "Cảm ơn cô"** trước khi về chỗ

---

# 📋 CHECKLIST TRƯỚC KHI THUYẾT TRÌNH

- [ ] Đọc script ít nhất 3 lần
- [ ] Chạy thử 1 lần có đo thời gian
- [ ] PowerPoint đã có đủ 18 slides
- [ ] Hình ảnh đã chèn vào slides (Silhouette, PCA, Algorithm comparison)
- [ ] Bảng 10 luật đã có trong slide 6
- [ ] In script ra giấy
- [ ] Laptop sạc đầy pin
- [ ] File PowerPoint save USB dự phòng
- [ ] Thuộc câu trả lời 10 câu hỏi phản biện
